...
-- Step 1 joining tables - code from slides
...
-- Step 2 : my version with additional columns
SELECT visits.record_id, visits.location_id AS visit_location,
...
-- Step 3 change names and remove a duplicate (location)
SELECT
	visits.record_id, 
    visits.location_id,
...
-- Step 4: checking if the scores are the same for the auditor and surveyor
SELECT visits.record_id,  visits.location_id,
    water_quality.subjective_quality_score AS surveyor_score, 
    auditor_report.true_water_source_score AS auditor_score
FROM  visits
JOIN water_quality ON visits.record_id = water_quality.record_id
...
-- Step 4 : to count the number of rows without the duplicates use a subquery = 1518 BUT with duplicates (remove visit_count=1) equals to 2505*/
SELECT COUNT(*) AS row_count
FROM (
    SELECT
        visits.record_id, visits.location_id,
        water_quality.subjective_quality_score AS surveyor_score, 
        auditor_report.true_water_source_score AS auditor_score
    FROM visits
    JOIN water_quality ON visits.record_id = water_quality.record_id
...
-- Step 5: finding the source of the different issues
SELECT visits.record_id,  visits.location_id,
    water_quality.subjective_quality_score AS surveyor_score, 
    auditor_report.true_water_source_score AS auditor_score,
...
JOIN water_quality ON visits.record_id = water_quality.record_id
JOIN auditor_report ON visits.location_id = auditor_report.location_id
JOIN water_source ON water_source.type_of_water_source = auditor_report.type_of_water_sourc
WHERE  subjective_quality_score <> true_water_source_score
AND visits.visit_count = 1;

-- Task 2: LINKING RECORDS
-- Step 1 join assigned employee id  from visits table to query
SELECT visits.record_id,  visits.location_id,
    water_quality.subjective_quality_score AS surveyor_score, 
    auditor_report.true_water_source_score AS auditor_score,
...
-- Step 3: change ID to employee names 
SELECT visits.record_id, visits.location_id,
    water_quality.subjective_quality_score AS surveyor_score, 
    auditor_report.true_water_source_score AS auditor_score,
...
-- Step 5: Query it by looking for distinct list of employees and counting how many there are */
WITH Incorrect_records AS( SELECT visits.record_id, visits.location_id,
						water_quality.subjective_quality_score AS surveyor_score, 
						auditor_report.true_water_source_score AS auditor_score,
						employee.employee_name
...
-- Step 6: Query it by looking for distinct list of employees and adding up how many mistakes they each made */
WITH  Incorrect_records AS(SELECT visits.record_id, visits.location_id,
						water_quality.subjective_quality_score AS surveyor_score, 
						auditor_report.true_water_source_score AS auditor_score,
						employee.employee_name
...
-- Step 1: Create view so that the code is less messy
CREATE VIEW Incorrect_records AS(SELECT visits.record_id, visits.location_id,
						water_quality.subjective_quality_score AS surveyor_score, 
						auditor_report.true_water_source_score AS auditor_score,
						employee.employee_name
					FROM visits
...
-- Step 2: counts how many mistakes each employee made*/
...
SELECT employee_name, location_id, statements
FROM Incorrect_records
WHERE statements IN (SELECT statements
						FROM incorrect_records
